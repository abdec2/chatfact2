...
$(document).on('click','[data-toggle="btnqchat"]',function(){
    ...
    var h_main = $('#main-container').height();
...
function checkfriendlist(buddyid) {
    var friendid = buddyid;

    $.ajax({type: "POST", url: base  + "chat/checkfriend", data: {friendid: friendid},cache: false,
...
$(document).on('click', '[data-toggle="popover"]', function(){
        ...
        var h_title = $("#chat-box > .chat-box-header").height();
        var top = '100px' /*($('#chat-box').is(':visible') ? (offset.top - h_title - 40) : (offset.top + h_title - 20))*/;
        if((top + $('#chat-box').height()) > h_main){ top = h_main -  $('#chat-box').height();}
        $('#chat-box').css({'top': top});
        if(!$('#chat-box').is(':visible')){
            $('#chat-box').toggle('slide',{
        ...
        if(message !== "" && e.which == 13){
            ...
            $.ajax({ type: "POST", url: base  + "chat/save_message", data: {message: message, user : user},cache: false,
                success: function(response){
                    ...
                    li = '<li class=" bubble '+ msg.type +'"><img src="'+base+'assets/images/thumbs/'+msg.avatar+'" class="avt img-responsive">\
                    <div class="message">\
...
$(document).on('click','#btnDel', function() {
    ...
    var r = confirm("Are you sure you want to delete this chat?");
    if (r == true){
        $.ajax({ 
            type: "POST", 
            url: base  + "chat/delete_chat",
            data:{buddy: buddy},
            cache: false,
            success: function(response){
                if(response.success)
                ...
                {
                    alert('Can not delete this chat...');
                }
                        ...
                        if($("#chat-box").is(":visible")){
                            chatbuddy = $("#chat_buddy_id").val();
                                if(this.sender == chatbuddy){
                                  li = '<li class="'+ this.type +'"><img src="'+base+'assets/images/thumbs/'+this.avatar+'" class="avt img-responsive">\
                                    <div class="message">\
                                    <span class="chat-arrow"></span>\
                                ...
                                update_lastseen();
                                    ...
                                    $.each(senders, function() {
                                        if(this.user == from){
                                            var receiver = $(".chat-group").find('span[rel="'+from+'"]');
                                            if (receiver.length == 0)
                                            {
                                                $.ajax({type:'POST',url: base + 'chat/checkfriend',data: {friendid: from},cache: false});
